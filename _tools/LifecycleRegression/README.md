...
- 后台线程替换播放列表时，仅在模拟 UI 队列排空后发布命令通知；旧列表解绑、已排队回调在 Dispose 后失效。桩属性保留真实 PropertyChanged 行为。
- IPC 未完成时保留最新显式 Play/Pause 意图，退出后不执行待处理意图。
- 监视关闭时不读取目录、不创建原生监视器；动态启停、扫描中关闭后立即重开、目录读取中退出均不会遗留或重建已停止的资源。
