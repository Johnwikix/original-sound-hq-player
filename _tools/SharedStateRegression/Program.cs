...
static async Task RunAsync()
{
    ...
    library.Library.Replace([new(10, "local") { SourceId = 0 }, new(11, "remote-a") { SourceId = 1 }, new(12, "remote-b") { SourceId = 2 }]);
    ...
    await projections.UpdateSongCollectionsAsync(list, SongViewType.All);
    Check(list.Count == 2 && list.All(m => m.IsRemote), "All remote sources excludes local songs.");
    library.Browse.SourceFilterId = -1;
    queries.SetSourceFilter(-1);
    projections.UpdateGroupedByFirstLetter(m => m.Album, m => "A", library.LibraryViews.AlbumPageSource);
    await Task.Yield();
    Check(((List<MusicGroup>)library.LibraryViews.AlbumPageSource.Source!).Sum(g => g.Items.Count) == 3, "Identical album names remain distinct between sources.");
    library.Browse.SearchText = "missing";
