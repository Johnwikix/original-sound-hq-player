...
internal static unsafe partial class Program
{
    private static void RunSurroundTests(string root)
    {
        Run("5.1: settings append is optional and defaults off for old IPC", () =>
        {
            ...
            Require(!BinarySerializer.ReadIpcSetting(bytes.AsSpan(0, length - 1)).ExperimentalSurround51, "old payload enabled experiment");
            Require(BinarySerializer.ReadIpcSetting(bytes.AsSpan(0, length - 1)).WasapiEndpointId == "endpoint", "old endpoint lost");
        });
