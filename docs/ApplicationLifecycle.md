...
2. 协议记录独立于生命周期。音频进程与 IPC 连接在 `StartAsync` 最前（先于协议）启动，与数据库初始化、协议阅读并行；协议确认后转为 Initializing，进行 SMTC 初始化、设置同步与文件监视。
3. 引擎轨在 IPC 连接、许可与缓存库（含播放状态恢复）汇合后推送首曲与初始音频设置，主界面不等待引擎就绪，推送完成前播放入口保持置灰（`IsPlaybackEngineReady`）。更新日志在交互启用后后台弹出，用户操作直到 Ready 才开放。
...
- 实际 MSIX 开发包（2026-09-18 改序前验证）：协议期间托盘存在、当时无音频子进程；拒绝与关闭退出；临时已确认夹具经过更新日志进入主界面；普通关闭后主进程与音频子进程均结束。夹具已移除，保持测试前没有协议记录的状态。改序后（音频进程先于协议拉起）的实机复验尚未执行。
