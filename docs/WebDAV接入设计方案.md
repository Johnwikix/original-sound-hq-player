...
“查看歌曲”进入现有音乐库并设置 SourceId 过滤；“浏览目录”展示按展开加载的服务端目录树，可多选已入库歌曲并按选择顺序播放。来源页负责管理，音乐库负责听歌。

### 3.2 添加 WebDAV

第一步填写连接信息：名称、WebDAV 根地址、用户名、密码/应用密码。“服务类型”可选自动/普通 WebDAV/AList 或 OpenList，仅用于提示和诊断，不依赖供应商私有 API 才能工作。

第二步“连接并选择目录”：用 PROPFIND 验证目录权限，按展开加载目录树，允许选择一个或多个任意深度的音乐根目录；编辑时沿已保存路径恢复选中状态。保存地址规范化结果，保留服务端返回的真实路径；同一来源的父子根重复选择要去重。
