...
在设置的“存储与缓存”区域增加“播放 WebDAV 歌曲时自动缓存”、缓存位置、容量上限、已用空间和清理按钮；与现有封面缓存分开。沿用 MVVM，开关和数值输入可放同一行，不给横向 ToggleSwitch 另加 Header。建议默认关闭、默认上限 10 GiB，默认目录使用应用 LocalCacheFolder 下专用 WebDavAudio 目录；这是本轮暂定参数，用户只确认了需要开关，未指定默认开/关。
...
- **设置**：关于页增加默认折叠的“WebDAV 播放缓存”卡片，包含自动缓存开关、GiB 上限、路径、占用和清理。无独立缓存设置分类。
...
| 验证 | 结果 |
|---|---|
| WebDAV 确定性核心回归 | 22 项通过：路径、重定向凭据边界、预算、取消、版本变化、跨窗口桥接、无 Range 降级、缓存开关/清理/完整文件保留、自研封面 |
...
最终界面另经实际开发安装包确认：音乐来源卡片统一布局；查看歌曲进入来源过滤后的歌曲页；来源图标列正常显示；网络播放图标与音频信息同一行，提示随打开/播放状态更新；关于页缓存卡片可折叠展开，开关与容量同排、路径采用左侧说明与右侧输入。测试开启的自动下载已恢复关闭，额度保留 10 GiB。
