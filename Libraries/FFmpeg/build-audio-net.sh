#!/usr/bin/env bash
# Run in MSYS2 UCRT64: bash build-audio-net.sh <clean FFmpeg n9.0.1 source> <build dir>
# Identical to build-audio.sh except: network enabled; protocols add http/https/tcp/tls;
# TLS backend is Windows-native SChannel (no external TLS library, LGPL-clean).
set -euo pipefail
source_dir=$(cd "$1" && pwd)
mkdir -p "$2"
cd "$2"
