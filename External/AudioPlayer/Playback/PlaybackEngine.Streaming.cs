...
public sealed partial class PlaybackEngine
{
    ...
    public StreamReply HandleStreamCommand(StreamCommand command)
    {
        ...
        lock (_streamLock)
        {
            if (_disposed != 0) return Reject(command, "Stopping");
            ...
            if (command.Method == "prepare")
            {
                ...
                if (_streamSlots.Count >= 2) return Reject(command, "PreparationLimit");
                try { command.Source.Validate(); }
                catch { return Reject(command, "InvalidSource"); }
                if (command.Source.IsLive) return Reject(command, "UnsupportedLiveSource");
            ...
            switch (command.Method)
            {
                ...
                case "seek":
                    ...
                    if (!slot.Source.CanSeek || seekSession is not { CanSeek: true } || command.PositionMs < 0)
                    ...
                    seekSession.PendingSeekId = command.SeekId;
                    seekSession.RequestSeek(command.PositionMs);
        ...
        slot.Work = Task.Run(async () =>
        {
            ...
            try
            {
                ...
                if (slot.ResumePosition > 0)
                {
                    ...
                    prepared.PendingSeekId = slot.SeekId;
                    prepared.RequestSeek(slot.ResumePosition);
                }
