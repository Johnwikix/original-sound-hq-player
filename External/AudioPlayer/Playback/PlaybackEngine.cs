...
public sealed partial class PlaybackEngine : IDisposable
{
    ...
    {
        if (_streamSlots.TryGetValue(_currentStream, out var remote) && remote.Source.Location == url)
        {
    ...
    public void PlayButton()
    {
        lock (_streamLock)
        {
            if (_requestedStream != _currentStream && _streamSlots.TryGetValue(_requestedStream, out var pending))
            {
                ...
                if (pending.WantsPlay && pending.Prepared is not null) CommitStream(pending);
                return;
            }
