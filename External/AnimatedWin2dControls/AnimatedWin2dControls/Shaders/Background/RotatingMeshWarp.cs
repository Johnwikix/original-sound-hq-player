...
public static class RotatingMeshWarp
{
    ...
    private const int DefaultPortraitControlPointCount = 6;
    private const int DefaultPortraitSubdivisionLevels = 2;
    private const int DefaultLandscapeControlPointCount = 9;
    private const int DefaultLandscapeSubdivisionLevels = 2;

    /// <summary>Row-major NDC positions of the "from" grid.</summary>
    public readonly record struct MeshData(Vector2[] From, Vector2[] To, int Rows, int Columns);

    /// <summary>Random shared instance for preset selection (mirrors the original behavior).</summary>
    private static readonly Random PresetRandom = new();

    /// <summary>
    /// Picks one of the five selectable slots; portrait maps slots 2 and 3 onto preset 2.
    /// </summary>
    ...
    public static MeshData Create(int presetIndex, bool isVerticalLayout)
    {
        ...
        var verticesFrom = new Vector2[rows * columns];
        var verticesTo = new Vector2[rows * columns];
        for (int row = 0; row < rows; row++)
        {
            ...
            for (int column = 0; column < columns; column++)
            {
                ...
                verticesFrom[index] = from[row, column] * 2f - Vector2.One;
                verticesTo[index] = to[row, column] * 2f - Vector2.One;

                // 'u'/'v' are implied by the regular grid layout (v = 1 at row 0),
                // so only the positions need to be uploaded.
                _ = u;
                _ = v;
            }
        }

        return new MeshData(verticesFrom, verticesTo, rows, columns);
    }
