...
通过 Microsoft Store 安装即可开始免费试用，试用期内全部功能开放（试用时长以商店页面为准）。试用结束后播放器仍可继续使用，以下完整版功能将被锁定：

- **进阶音效处理**：卷积校正、音量均一化、统一前级增益、声道与耳机效果（十段均衡器始终免费可用）
- **DSD 位流输出**：DoP 与 DSD Native
- **实验性输出**：5.1 多声道 PCM 输出、Atmos（E-AC-3/JOC）HDMI 位流直通（需兼容接收设备）

受限期间你的原有设置不会被修改或清除；在音效页提示条或「关于」页完成购买后，原设置自动恢复生效。非 Microsoft Store 渠道（含自行从源码构建）的版本不受上述限制。
...
- 🔊 **专业音频处理**
  ...
  - 卷积校正：绘制校正曲线（2–32 个可拖动控制点，支持保存、更新与删除命名预设）或导入耳机/房间校正 WAV 脉冲响应（IR），生成最小相位 FIR 实时卷积，按输出采样率自动重采样；编辑器内置 EQ 与卷积合成频响实时预览
